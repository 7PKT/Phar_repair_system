...
const RepairList = () => {
    ...
    const moveToCompletedCategory = async (repairId) => {
        try {
            ...
            let completedCategory = categories.find(cat => 
            ...
            if (!completedCategory) {
                ...
                const categoryResponse = await axios.post('/api/repairs/categories', 
            ...
            await axios.put(`/api/repairs/${repairId}`, 
...
<body>
    <div class="header">
        ...
        <p>ผู้ส่งออก: ${user?.full_name || user?.username || 'ไม่ระบุ'} (${user?.role === 'admin' ? 'ผู้ดูแลระบบ' : user?.role === 'technician' ? 'ช่างเทคนิค' : 'ผู้ใช้งาน'})</p>
    </div>
    ...
    <div class="summary">
        ...
        <div style="margin-top: 12px; padding-top: 12px; border-top: 1px solid #e5e7eb; font-size: 10px;">
            ...
            ความสำคัญ: ${priorityFilter === 'all' ? 'ทั้งหมด' : getPriorityText(priorityFilter)}, 
            หมวดหมู่: ${categoryFilter === 'all' ? 'ทั้งหมด' : categories.find(c => c.id == categoryFilter)?.name || 'ไม่ระบุ'}, 
        ...
        <tbody>
            ${exportData.map(repair => `
                <tr>
                    ...
                    <td class="center">
                        <span class="status-${repair.status}">${getStatusText(repair.status)}</span>
                    </td>
                    <td class="center">
                        <span class="priority-${repair.priority}">${getPriorityText(repair.priority)}</span>
                    </td>
    ...
    <div class="footer">
        ...
        <p><strong>หมายเหตุ:</strong> ข้อมูลนี้แสดงรายการแจ้งซ่อม${getViewModeText(activeView)}ในระบบ</p>
    </div>
</body>
</html>`;
            ...
            const fileName = `รายงานการแจ้งซ่อม_${getViewModeText(activeView)}_${new Date().toLocaleDateString('th-TH').replace(/\//g, '-')}.xls`;
    ...
    const getViewModeText = (view) => {
        switch(view) {
            case 'active': return 'รายการที่ยังดำเนินการ';
            case 'completed': return 'รายการที่เสร็จสิ้นแล้ว';
    ...
    const filteredRepairs = filteredRepairsByView.filter(repair => {
        ...
        return matchesSearch && matchesStatus && matchesPriority && matchesCategory && matchesTechnician && matchesDateRange;
    });
    ...
    const mobileHeaderContent = (
        <div className="flex items-center justify-between w-full">
            <div className="flex items-center space-x-2">
                ...
                >
                    <RefreshCw className={`w-3 h-3 sm:w-4 sm:h-4 ${isMobile ? '' : 'mr-1'} ${refreshing ? 'animate-spin' : ''}`} />
                    {!isMobile && 'รีเฟรช'}
                </button>
                ...
                {filteredAndSortedRepairs.length > 0 && (
                    ...
                    >
                        <FileSpreadsheet className={`w-3 h-3 sm:w-4 sm:h-4 ${isMobile ? '' : 'mr-1'}`} />
                        {!isMobile && 'Excel'}
                    </button>
                )}
            </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4 sm:p-6 hover:shadow-md transition-shadow">
            <div className="flex items-start justify-between mb-3">
                <div className="flex items-center space-x-2">
                    ...
                    <span className={`px-2 py-1 text-xs font-medium rounded-full border ${getStatusBadge(repair.status)}`}>
                        {getStatusText(repair.status)}
                    </span>
                </div>
                <div className="flex items-center space-x-1">
                    <span className={`px-2 py-1 text-xs font-medium rounded-full border ${getPriorityBadge(repair.priority)}`}>
            ...
            <h4 className="text-base sm:text-lg font-semibold text-gray-900 mb-2 line-clamp-2">{repair.title}</h4>
            ...
            <div className="flex flex-col sm:flex-row gap-2">
                ...
                {(user?.role === 'admin' || user?.role === 'technician') && repair.status === 'completed' && (
                    <button
            ...
            <div className="space-y-4 sm:space-y-6">
                ...
                <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4 sm:p-6">
                    <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between">
                        <div className="mb-4 lg:mb-0">
                            <h2 className="text-lg sm:text-xl font-semibold text-gray-900 mb-2">จัดการการแจ้งซ่อม - {getViewModeText(activeView)}</h2>
                            <p className="text-sm sm:text-base text-gray-600">
                                ดูและติดตามรายการแจ้งซ่อม{activeView === 'all' ? 'ทั้งหมด' : ''}ในระบบ
                                {user?.role === 'user' && (
                                    <span className="block text-xs sm:text-sm text-amber-600 mt-1">
                                        💡 คุณสามารถแก้ไขได้เฉพาะรายการที่คุณสร้างและยังอยู่ในสถานะ "รอดำเนินการ" เท่านั้น
                                    </span>
                                )}
                            </p>
                        </div>
                        <div className="text-left lg:text-right text-xs sm:text-sm text-gray-500">
                            ...
                            <p>บทบาท: {user?.role === 'admin' ? 'ผู้ดูแลระบบ' :
                                user?.role === 'technician' ? 'ช่างเทคนิค' : 'ผู้ใช้งาน'}</p>
                ...
                <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4 sm:p-6">
                    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                        ...
                        {activeView === 'completed' && (user?.role === 'admin' || user?.role === 'technician') && (
                            <div className="text-xs sm:text-sm text-green-600 bg-green-50 px-3 py-2 rounded-lg">
                                💡 รายการที่เสร็จสิ้นแล้วสามารถย้ายไป category "เสร็จสิ้น" เพื่อจัดเก็บได้
                            </div>
                        )}
                    </div>
                </div>
                ...
                {showMobileFilters && isMobile && (
                    <div className="fixed inset-0 z-50 bg-black bg-opacity-50" onClick={() => setShowMobileFilters(false)}>
                        <div className="fixed bottom-0 left-0 right-0 bg-white rounded-t-xl p-4 max-h-[80vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
                ...
                <div className="bg-white rounded-xl shadow-sm border border-gray-100">
                    <div className="p-4 sm:p-6 border-b border-gray-200">
                        ...
                        {(statusFilter !== 'all' || priorityFilter !== 'all' || categoryFilter !== 'all' ||
                            technicianFilter !== 'all' || dateRangeFilter !== 'all' || searchTerm || activeView !== 'all') && (
                                <div className="mt-4 p-3 bg-blue-50 rounded-lg">
                                    <p className="text-xs sm:text-sm text-blue-800">
                                        <strong>ผลการกรอง:</strong> แสดง {filteredAndSortedRepairs.length.toLocaleString('th-TH')} รายการ จาก {getViewModeText(activeView)} {filteredRepairsByView.length.toLocaleString('th-TH')} รายการ
                        ...
                        <div className="p-6 sm:p-8 text-center">
                            ...
                            <p className="text-sm sm:text-base text-gray-500 mb-2">
                                {searchTerm || statusFilter !== 'all' || priorityFilter !== 'all' ||
                                    categoryFilter !== 'all' || technicianFilter !== 'all' || dateRangeFilter !== 'all'
                                            ...
                                            <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
                                                <div className="flex-1 min-w-0">
                                                    <div className="flex items-center space-x-2 mb-2 flex-wrap gap-1">
                                                        {getStatusIcon(repair.status)}
                                                        <h4 className="text-base sm:text-lg font-medium text-gray-900 truncate">{repair.title}</h4>
                                                        <span className={`px-2 py-1 text-xs font-medium rounded-full border ${getStatusBadge(repair.status)}`}>
                                                            {getStatusText(repair.status)}
                                                        </span>
                                                        <span className={`px-2 py-1 text-xs font-medium rounded-full border ${getPriorityBadge(repair.priority)}`}>
                                                    ...
                                                    <p className="text-sm sm:text-base text-gray-600 mb-3 line-clamp-2">{repair.description}</p>
                                                    ...
                                                    <div className="flex flex-wrap items-center gap-2 sm:gap-4 text-xs sm:text-sm text-gray-500">
                                                        ...
                                                        <div className="flex items-center">
                                                            <Calendar className="w-3 h-3 sm:w-4 sm:h-4 mr-1 flex-shrink-0" />
                                                            <span>{new Date(repair.created_at).toLocaleDateString('th-TH', {
                                                                year: 'numeric',
                                                                month: 'short',
                                                                day: 'numeric'
                                                            })}</span>
                                                ...
                                                <div className="flex flex-row sm:flex-col gap-2 sm:ml-4">
                                                    ...
                                                    {(user?.role === 'admin' || user?.role === 'technician') && repair.status === 'completed' && (
                                                        <button
                                                            onClick={() => moveToCompletedCategory(repair.id)}
                                                            className="flex items-center justify-center px-3 py-2 text-xs sm:text-sm bg-purple-100 text-purple-700 rounded-lg hover:bg-purple-200 transition-colors whitespace-nowrap"
                                                            title="ย้ายไป category เสร็จสิ้น"
                                                        >
                                                            <Archive className="w-3 h-3 sm:w-4 sm:h-4 mr-1" />
                                                            เก็บเข้าคลัง
                        ...
                        <div className="p-4 sm:p-6 border-t border-gray-200">
                            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                                <div className="text-xs sm:text-sm text-gray-600 text-center sm:text-left">
                                    แสดง {(startIndex + 1).toLocaleString('th-TH')}-{Math.min(endIndex, filteredAndSortedRepairs.length).toLocaleString('th-TH')} จาก {filteredAndSortedRepairs.length.toLocaleString('th-TH')} รายการ
                                </div>
                ...
                {/* Quick Actions Summary for Completed Items */}
                {activeView === 'completed' && (user?.role === 'admin' || user?.role === 'technician') && (
                    <div className="bg-gradient-to-r from-green-50 to-blue-50 rounded-xl shadow-sm border border-green-200 p-4 sm:p-6">
                        <div className="flex items-center justify-between">
                            <div>
                                <h3 className="text-base sm:text-lg font-semibold text-green-900 mb-2">
                                    📋 จัดการรายการที่เสร็จสิ้น
                                </h3>
                                <p className="text-sm text-green-700">
                                    คลิกปุ่ม "เก็บเข้าคลัง" เพื่อย้ายรายการที่เสร็จสิ้นไปยัง category "เสร็จสิ้น" 
                                    เพื่อการจัดเก็บและค้นหาที่ดีขึ้น
                                </p>
                            </div>
                            <div className="flex items-center space-x-2">
                                ...
                                <div className="px-3 py-2 bg-green-100 rounded-lg">
                                    <span className="text-sm font-medium text-green-800">Category: เสร็จสิ้น</span>
                                </div>
                            </div>
                        </div>
                    </div>
