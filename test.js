...
const RepairForm = () => {
    ...
    const fetchBuildings = async () => {
        try {
            ...
            const response = await axios.get('/api/rooms/buildings', {
                headers: {
                    Authorization: `Bearer ${token}`,
                    'Content-Type': 'application/json'
                }
            ...
            if (error.response?.status === 401) {
                ...
                toast.error('เกิดข้อผิดพลาดในการโหลดข้อมูลอาคาร');
            }
        } finally {
    ...
    const fetchFloors = async (building) => {
        try {
            ...
            const response = await axios.get(`/api/rooms/floors?building=${building}`, {
                headers: {
                    Authorization: `Bearer ${token}`,
                    'Content-Type': 'application/json'
                }
            ...
            if (response.data.success) {
                ...
                setFloors([]);
                console.warn('No floors found for building', building);
            }
        } catch (error) {
            ...
            if (error.response?.status === 401) {
                ...
                toast.error('เกิดข้อผิดพลาดในการโหลดข้อมูลชั้น');
            }
        }
    };
    ...
    if (categoriesLoading || buildingsLoading) {
        return (
